...
This guide explains how to set up multi-search for global use, MCP integration, and testing.
...
```bash
cd /Volumes/Storage/code/multi-search
bun install
```
...
The sample config is already at `multi-search.config.json`. It includes:
...
If you don't want to use local SearXNG, edit `multi-search.config.json`:
...
multi-search "search query" --json
```
...
```json
{
  "mcpServers": {
    "multi-search": {
      ...
      "args": [
        "run",
        "/Volumes/Storage/code/multi-search/mcp-server.ts"
      ],
      "env": {
...
```typescript
import { multiSearch, getCreditStatus } from 'multi-search';
...
Disable SearXNG in `multi-search.config.json`:
```json
...
│   └── config/
│       └── multi-search.config.json  # Sample config
└── multi-search.config.json  # Active config
```
