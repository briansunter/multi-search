...
# Ensure you're in the multi-search directory
cd /Users/briansunter/.dotfiles/bun-scripts/multi-search
...
cat multi-search.config.json | grep -A 2 '"defaultEngineOrder"'
# Expected output should show: "searxng", "tavily", "brave", "linkup"

# Check SearXNG configuration
cat multi-search.config.json | grep -A 15 '"id": "searxng"'
# Should show:
# - enabled: true
# - autoStart: true
# - autoStop: true
...
```bash
# Use valid config
bun run src/cli.ts "valid config test" --config multi-search.config.json
