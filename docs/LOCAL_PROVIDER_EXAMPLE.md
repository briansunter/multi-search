...
This guide shows step-by-step how to add a local Docker-based embedding provider to multi-search using the improved abstraction.
...
services:
  embedding-service:
    ...
    container_name: multi-search-embedding
    ports:
      - "8000:8000"
    environment:
  ...
  async search(query: SearchQuery): Promise<SearchResponse> {
    ...
    if (!isHealthy) {
      throw new SearchError(
        ...
        "Embedding service is not healthy. Run: multi-search providers init",
      );
    }
  ...
  "engines": [
    {
      ...
      "containerName": "multi-search-embedding",
...
echo ""
echo "Install with: multi-search providers init"
echo "Search with: multi-search 'your query'"
```
...
# Starting Docker container: multi-search-embedding
...
# Expected output:
# Stopping Docker container: multi-search-embedding
```
  ...
  "engines": [
    {
      ...
      "containerName": "multi-search-minilm",
...
multi-search "query" --engines local-minilm

# Search with E5 (better quality, slower)
multi-search "query" --engines local-e5

# Compare both
multi-search "query" --engines local-minilm,local-e5 --json
```
...
# Check logs
multi-search providers logs embedding-service
...
docker ps | grep multi-search-embedding

# Check health
multi-search providers health
...
# Check configuration
multi-search providers validate local-embeddings
```
