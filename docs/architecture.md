...
- **Config resolution**: `src/config/load.ts` searches explicit path → local `multi-search.config.(ts|json)` → XDG config. TypeScript configs use `defineConfig` helpers; JSON validated against `config.schema.json`.
...
- **Orchestration**: `MultiSearchOrchestrator` executes strategies from `src/core/strategy/*`, collects attempts, and returns normalized results with credit snapshots.
...
- Credit state defaults to `~/.local/state/multi-search/credits.json`; override via config or CLI options that accept `creditStatePath` through bootstrap options.
